...
WORKDIR = "./tmp"
MODEL_NAME = "probe_cpu_bert_qkv_static_prepack"
...
def is_static_fc_param(logical_name):
    return (
        ".mha.qkv." in logical_name
        or ".mha.proj." in logical_name
...
def set_nonstatic_constants(module, encoder):
    ...
    supported_static_fc_ops = {
        ...
        # Compiler op name is normally generic; m2n3 is the
        # backend specialization. Accept both for robustness.
    ...
    for logical_name, param in encoder.named_parameters():
        ...
        if len(static_fc_dst_ops) > 1:
            raise RuntimeError(
                "Expected at most one static-FC destination op "
                f"for {logical_name}, got "
                f"{[(op._attrs.get('op'), op._attrs.get('name')) for op in static_fc_dst_ops]}"
            )

        if len(static_fc_dst_ops) == 1:
            op = static_fc_dst_ops[0]
            ...
            if logical_name.endswith(".weight"):
                ...
                raise RuntimeError(
                    "Static FC parameter is neither weight nor bias: "
                    f"{logical_name}"
                )
            ...
            static_groups.setdefault(base, {})[kind] = {
                ...
                "op_type": op._attrs.get("op"),
            }

            continue

        # Everything else remains a normal constant.
        value = torch.empty(
            shape,
            dtype=torch.float32,
        )
...
def main():
    ...
    expected_static_pairs = NUM_LAYERS * 4

    if len(static_groups) != expected_static_pairs:
        print()
        print("Detected static FC groups:")
        for base in sorted(static_groups):
            group = static_groups[base]
            info = group.get("weight") or group.get("bias")
            print(
                " ",
                base,
                "->",
                info.get("op_type"),
                info.get("prepack_symbol"),
            )

        raise RuntimeError(
            "Expected exactly "
            f"{expected_static_pairs} static FC pairs "
            "(QKV + projection + FFN1 + FFN2 for each layer), "
            f"got {len(static_groups)}"
        )

    qkv_groups = [
        base
        for base in static_groups
        if ".mha.qkv" in base
    ]

    if len(qkv_groups) != NUM_LAYERS:
        raise RuntimeError(
            f"Expected {NUM_LAYERS} QKV static groups, "
            f"got {len(qkv_groups)}: {sorted(qkv_groups)}"
        )
    ...
    expected_old_static_bytes = int(
        243.2109375 * 1024 * 1024
    )

    expected_total_bytes = (
        expected_old_static_bytes
        + expected_qkv_bytes
    )

    if abs(total_raw_bytes - expected_total_bytes) > 4096:
        raise RuntimeError(
            "Unexpected total static raw size: "
            f"{total_raw_bytes / 1024 / 1024:.3f} MiB; "
            f"expected about "
            f"{expected_total_bytes / 1024 / 1024:.3f} MiB"
        )
